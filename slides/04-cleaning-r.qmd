...
df <- tibble::tribble(~id, ~test_score, ~grade_level,
                1, 350, 3,
                2, 380, 4,
                3, 290, 5)
...
3. Update the code and run the code to read in the data using the green arrow.
4. Run the code chunk to view the data.
...
df <- data.frame(name = c("emma", "brandon"),
                 score = c(22, 40))
...
Using [tidyselection](https://tidyselect.r-lib.org/reference/language.html) helpers
```{r}
#| echo: true
#| eval: false
...
write_csv(df, file = "data/raw/w1_stu_obs_clean.csv", na = "")
